...
int loadrules(FILE *fp, vector<pc_rule> &classifier, int format) {
    int i = 0;
    int wild = 0;
    ...
    while(1) {
        ...
        if(format == 1){
            if(fscanf(fp,"@%u.%u.%u.%u/%u\t%u.%u.%u.%u/%u\t%llu : %llu\t%llu : %llu\t%x/%x\t%x/%x\t%d\n",
                        ...
                        &proto, &protomask, &junk, &junkmask, &rule.priority) != 19) break;
        }
        else if(format == 2) {
...
range range_in_boundary_1D(const range &r, const range &boundary)
{
    range ret;
...
void remove_redund_rt(vector<pc_rule*> &pr, rule_boundary &b) 
{
...
void remove_redund(vector<pc_rule*> &pr, rule_boundary &rb)
{
    vector <pc_rule*> rulelist;
    int i = 0;
...
int linear_search(vector<pc_rule*> &p_classifier, field_type *ft)
{
    int ret = -1;
    for(auto it = p_classifier.begin();
            it != p_classifier.end();
            it++) {
...
void extend_rules(vector<pc_rule> &in, vector<pc_rule> &out) 
{
    ...
    cout<<"Output ruleset: "<<out.size()<<endl;
}
