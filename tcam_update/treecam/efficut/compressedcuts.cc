...
void NodeStats(node *curr_node)
{
  ...
  if (Actual_Curr_Level >= Max_Levels)
  {
    ...
    if (curr_node->children.empty()) {
        if(curr_node->classifier.size() > Rules_at_the_Leaf)
            Rules_at_the_Leaf = curr_node->classifier.size();
    }
...
void output_rules()
{
    ...
    for (int i = 0; i < 5; i++) {
      if (!(bigrules[i].empty())) {
          sprintf(filename, "f%d", rs_count++); 
          dump_rules(bigrules[i], string(filename));
      }
    }
    for (int j = 0; j < 10; j++) {
      if (!(kindabigrules[j].empty())) {
          sprintf(filename, "f%d", rs_count++); 
          dump_rules(kindabigrules[j],string(filename));
      }
    }
    for (int k = 0; k < 10; k++) {
      if (!(mediumrules[k].empty())) {
          sprintf(filename, "f%d", rs_count++); 
          dump_rules(mediumrules[k], string(filename));
      }
    }
    for (int l = 0; l < 5; l++) {
      if (!(littlerules[l].empty())) {
          sprintf(filename, "f%d", rs_count++); 
          dump_rules(littlerules[l], string(filename));
      }
    }
    if (!(smallrules.empty())) {
          sprintf(filename, "f%d", rs_count++); 
          dump_rules(smallrules, string(filename));
    }
    exit(-1);
